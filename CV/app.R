...
# 
# #Carpetas
# Regiones<-read.csv("IDM_NM_ago22.csv", encoding="latin1", check.names = T) %>%
...
# municipios_2<-read.csv("IDM_NM_ago22.csv",check.names = T, encoding = "latin1") %>%
...
# 
# 
# read.csv("IDM_NM_jul22.csv",check.names = T, encoding = "latin1") %>%
#   filter(Entidad=="Jalisco",
#          Subtipo.de.delito %in% c("Abuso sexual", "Acoso sexual", "Hostigamiento sexual",
#                                   "Violación simple", "Violación equiparada", "Feminicidio",
#                                   "Violencia familiar",
...
# victimas <- read.csv("IDVFC_NM_ago22.csv", encoding="latin1", check.names = T) %>%
...
# ---------------------------------------------------------------------------- #  
  ...
  output$grafico_violencia_familiar <- renderPlotly ({
    ...
    ggplotly(grafico_violencia_familiar_1) %>%
      layout(title = list(text = paste0("Total de denuncias por violencia familiar \n",
...
# ---------------------------------------------------------------------------
  ...
  output$grafico_Regiónal<- renderPlotly({
    ...
    ggplot(Regiones_data()) +
      aes(x = as.numeric(Año), y = Total, color = Región, fill= Región) +
  ...
  output$grafico_municipal<- renderPlotly({
    ...
    ggplot(municipios_data()) +
      aes(x =as.numeric(Año), y = Total, color = Municipio, fill= Municipio) +
...
output$total_atenciones_grafico <- renderPlotly({
  ...
  ggplotly(grafico_total_atenciones) %>%
    layout(legend = list(orientation = "h", x = 0.1, y = -0.3),
           margin = list(b=0,t=30),
...
output$atenciones_mensuales<- renderPlotly({
  ...
  #atenciones %>%     
    ...
    ggplot()+
    aes(x = reorder(tipo, total), y = total, fill=tipo) +
    geom_col() +
    scale_fill_manual(
      values = c(
        Psicológica = "#6737ab",
  ...
  ggplotly(grafico_anteciones_mensuales) %>%
    layout(legend = list(orientation = "h", x = 0.1, y = -0.3),
           margin = list(b=0,t=30),
           title = paste0("Total de atenciones realizadas\n", 
                          atenciones_reactive()$Unidad, 
...
output$atenciones_tipo<- renderPlotly({
  ...
  ggplotly(grafico_anteciones_tipo) %>%
    layout(legend = list(orientation = "h", x = 0.1, y = -0.3),
           margin = list(b=0,t=30),
           title = paste0("Total de atenciones realizadas\n", 
                          atenciones_reactive()$Unidad, 
